...
passport.use(new LocalStrategy((username, password, done) => {
  ...
  crypto.scrypt(password, user.salt, 32, (err, derivedKey) => {
    ...
    return done(null, { id: user.id, username: user.username });   // Hash e salt restano nel DB.
  });
}));

// In sessione tengo solo l'id
...
// La sessione gestisce il cookie e mette a disposizione req.session.
app.use(session({
  secret: 'ultima-corsa-secret',
  resave: false,
  saveUninitialized: false,
}));
// Passport si appoggia alla sessione.
...
app.post('/api/games', isLoggedIn, (req, res) => {
  ...
  // Distanze BFS da start: candidates a >= 3 tratte, mai vuoto
...
app.post('/api/games/:id/route', isLoggedIn, (req, res) => {
  ...
  // I 90 secondi vanno verificati qui.
  ...
  if (!valid) {
    // Percorso invalido o fuori tempo
